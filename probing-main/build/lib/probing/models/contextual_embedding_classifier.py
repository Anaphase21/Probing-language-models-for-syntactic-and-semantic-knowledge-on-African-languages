...
from transformers import AutoModel, AutoConfig, MT5EncoderModel, XLMRobertaModel, AutoModelForMaskedLM, AutoModelForTokenClassification, XLMRobertaForMaskedLM
    ...
    def load_base_model(self, model_name, randomize_embedding_weights, randomize_transformer_layers):
        ...
        if randomize_embedding_weights:
            ...
            if "Davlan" in model_name:
                self.embedder = AutoModelForMaskedLM.from_pretrained(model_name, config=self.config)
            elif "bonadossou" in model_name:
                self.embedder = XLMRobertaModel.from_pretrained(model_name, config=self.config)
            elif "castorini" in model_name:
                self.embedder = AutoModelForTokenClassification.from_pretrained(model_name, config=self.config)
            elif "francois-meyer" in model_name:
                self.embedder = XLMRobertaForMaskedLM.from_pretrained(model_name, config=self.config)
            else:
